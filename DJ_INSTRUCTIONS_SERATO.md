...
**Status: DRAFT**, matches the current build (`plan/12-mvp-v0.md`). Exact menu wording in Serato may differ slightly by version — this will get more precise once verified on a Mac. No coding knowledge needed for anything below.
...
*(This section is a placeholder until there's a proper installer — for now, "downloading" means getting the built plugin files from wherever they were built and placing them by hand. Will be simplified once packaging is done.)*

1. Get the two plugin files: `YuVi Glow.vst3` and `YuVi Glow.component`.
2. Place them here:
   - `YuVi Glow.vst3` → `~/Library/Audio/Plug-Ins/VST3/`
   - `YuVi Glow.component` → `~/Library/Audio/Plug-Ins/Components/`
3. If Serato was already open, restart it so it rescans for new plugins.
...
4. **Map your pads/knobs/faders**: click "Edit MIDI Mapping" — every pad/knob/fader turns red. Just start touching the ones you want to use (in any order) and they'll turn green as they're captured. Click "Edit MIDI Mapping" again when you're done.
...
- The "Input Gain" slider controls how loud the *deck's own sound* is as it passes through the plugin — leave it centered for normal volume. Toggle "Lock @ 0dB" if you want to guarantee the deck's volume can never be accidentally changed by a bumped fader.
...
Nothing here is locked to the Code 49 or MPD226 specifically — the "Edit MIDI Mapping" step works with any MIDI controller. If you're using something else, just skip straight to that step.
