...
**Update, same day**: the "can't verify FX-slot hosting from Linux" gap below is now partially closed — Mixxx is a real, free, cross-platform DJ app that runs on Linux, so the *general* "does this plugin work correctly when a real DJ app hosts it as an FX-slot effect, feeds it deck audio, and the plugin also listens to a MIDI controller directly" question can be tested via Mixxx today. See `DJ_INSTRUCTIONS_MIXXX.md`. **Correction, 2026-08-13**: this originally said Mixxx "hosts VST3 effects directly" — wrong, checked properly and Mixxx has no VST/VST3 support at all. It hosts **LV2** instead, which is why YuVi Glow gained an LV2 build target (`plan/issues/20-mvp-beta.md`) — the underlying point here still holds, just via LV2. What Mixxx *can't* answer is anything Serato-specific (its exact plugin format support, its exact FX-panel UI, its own MIDI-mapping interplay) — those still need the Mac.
