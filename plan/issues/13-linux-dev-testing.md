...
**Status: [COMPLETED]** — this workflow is real and has been used continuously throughout development (`scripts/linux_dev_env.sh`, the Standalone smoke-test loop).
...
```
build/YuViGlow_artefacts/Standalone/YuVi Glow
```
(No `Debug/` subfolder — that nesting only happens with multi-config generators like Xcode/MSVC. The default here, Unix Makefiles, is single-config, so the artifact sits directly under `Standalone/`. This was a real bug in `scripts/linux_dev_env.sh` until it got fixed mid-session — see that script for the belt-and-suspenders "try both paths" fix.)
